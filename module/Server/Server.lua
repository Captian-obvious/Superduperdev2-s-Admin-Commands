...
function server:GetCommand(str)
    ...
    if str:sub(1, 2):find("/e") then
        ...
        for i, v in pairs(server.Commands) do
            if v and v.Prefix then
                ...
                warn(
                    "Command is nil (" ..
                        i .. " | Cmd Table: " .. tostring(v) .. " | Pre: " .. tostring(v.Prefix) .. ")"
                )
            end
        end
    else
    ...
    if thecommand then
        ...
        warn(
            "Superduperdev2 Admin Commands: Could not find command! | String: " ..
                tostring(str) .. " | Args: " .. tostring(args) .. " | Cmd: " .. tostring(thecommand)
        ) -- using tostring bc they might be nil
        return nil
    end
end
